...
* Get a new access token from EGI Check-in according to instructions from `FedCloud Check-in
  ...
  Note: Do not supply refresh tokens as access tokens. The access tokens are generated by executing the command in
  the last line of https://aai.egi.eu/fedcloud, in the form of ``curl -X POST -u...``
